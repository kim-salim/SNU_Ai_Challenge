...
def _prepare_inputs(processor: Any, prompt: str, frames: list[Any], device: torch.device | str) -> dict[str, Any]:
    if len(frames) != 4:
    ...
    *,
    ...
    with torch.no_grad():
        for idx, row in enumerate(rows):
            ...
            inputs = _prepare_inputs(processor, prompt, frames, device)
