...
cd "$ROOT_DIR"
mkdir -p logs outputs/predictions/qwen25vl_7b_24candidate/valid_a_full
...
CMD=(
  ...
  --image-root data/raw
  --output-dir outputs/predictions/qwen25vl_7b_24candidate/valid_a_full
  --max-samples -1
...
echo "${CMD[@]}"
"${CMD[@]}" 2>&1 | tee logs/qwen25vl7b_valid_a_full.log
