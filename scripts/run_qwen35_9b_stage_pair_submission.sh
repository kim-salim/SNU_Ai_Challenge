...
LOG_DIR="$ROOT_DIR/logs/qwen35_9b_stage_pair"
RUN_ID="${RUN_ID:-qwen35_9b_stage_pair_qlora}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/outputs/experiments/qwen35_9b_stage_pair/$RUN_ID}"
mkdir -p "$LOG_DIR" "$OUT_DIR"

PYTHON_BIN="${PYTHON_BIN:-python3}"
CONFIG="${CONFIG:-configs/exp/qwen35_9b_stage_pair_qlora.yaml}"
CHECKPOINT="${CHECKPOINT:-weights/qwen35_9b_stage_pair/$RUN_ID/best}"
...
echo "${CMD[*]}"
"${CMD[@]}" 2>&1 | tee "$LOG_DIR/submission_${RUN_ID}.log"
