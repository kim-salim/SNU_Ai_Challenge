...
OUTPUT="${OUTPUT:-${ROOT}/artifacts/qwen35_27b_int4_port/05_runtime_smoke/synthetic_smoke.json}"
...
if [[ -n "${MIGRATED_HEADS:-}" ]]; then ARGS+=(--migrated-heads "${MIGRATED_HEADS}"); fi
"${PYTHON_BIN}" -m snu_order.qwen3vl.smoke_qwen35_27b_int4 "${ARGS[@]}"
